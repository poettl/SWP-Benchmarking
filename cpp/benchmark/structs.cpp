...
#include "../bubbleSort/header/bubbleSortStructs.h"
#include "../insertionSort/header/insertionSortStructs.h"
#include "../radixSort/header/radixSortStructs.h"
#include "../Person.h"
...
const size_t personsInCache = cacheSize / sizeof(Person);
...
template <typename R>
void populateDataSet(vector<Person> &data, R rng)
{
	for (Person &d : data){
        d.age = rng();
        d.name = "TEST " + rng();
...
int main()
{
	...
	// Our test data set
	auto data = vector<Person>(personsInCache - 1);
	...
	for (unsigned int i = 0; i < iterations; ++i)
	{
		...
		vector<Person> bubbleSortVector = data;
		Person *bubbleSortArr = &bubbleSortVector[0];
		...
		vector<Person> insertionSortVector = data;
		Person *insertionSortArr = &insertionSortVector[0];
		...
		vector<Person> radixSortVector = data;
		Person *radixSortArr = &radixSortVector[0];
